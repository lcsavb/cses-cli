...
def save_config(root_directory):
    """Saves the root directory to a configuration file in the script's directory."""
...
def load_config():
    """Loads the root directory from the configuration file if it exists."""
...
def get_root_directory(cli_root_directory):
    """Determines the root directory to use. 
    - If provided via CLI (`-r`), it is saved and used.
    - If a config file exists, it is loaded.
    - If neither, it prompts the user and saves the value.
    """
    if cli_root_directory:
        save_config(cli_root_directory)
        return cli_root_directory

    root_directory = load_config()
    if not root_directory:
        root_directory = input("Enter the root directory to store test cases: ").strip()
        save_config(root_directory)
...
"""
    ...
    parser = argparse.ArgumentParser(description="Download and extract test cases from CSES", add_help=False)
    parser.add_argument("-t", "--task_number", help="The CSES task number")
    parser.add_argument("-f", "--directory_name", help="Task's subdirectory name e.g. hanoi")
    parser.add_argument("-u", "--username", help="Your CSES username")
    parser.add_argument("-p", "--password", help="Your CSES password")
    parser.add_argument("-r", "--root_directory", help="CSES root directory (only needed the first time)")
    parser.add_argument("-h", "--help", action="store_true", help="Show this help message and exit")

    args = parser.parse_args()

    if args.help or not any(vars(args).values()):  # Show help if -h is used or no arguments are provided
        print(help_text)
        sys.exit(0)

    root_directory = get_root_directory(args.root_directory)

    run(args.task_number, root_directory, args.directory_name, args.username, args.password)
